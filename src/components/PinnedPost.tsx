...
export function PinnedPost({
  ...
  const [userLoaded, setUserLoaded] = useState(providedUser !== undefined && providedUser !== null);
  const splashEnabled = showSplash === true;
  const [mask, setMask] = useState<boolean>(splashEnabled);
  const [readyAt] = useState<number>(() => Date.now() + (splashEnabled ? MIN_SPLASH_DURATION_MS : 0));

  console.log('[PinnedPost] render start', { initialTab, hasUser: !!currentUser, userLoaded });

  const hideMask = () => {
    setMask(prev => {
      if (!prev) return prev;
      console.log('title: hiding');
      return false;
    });
  };

  const loadPinnedData = async () => {
    console.log('[PinnedPost] loadData: start');
    ...
    try {
      ...
      if (!userLoaded) {
        const { currentUser: fetchedUser } = await hydrateCurrentUser(context, service);
        setCurrentUser(fetchedUser);
        setUserLoaded(true);
      }
      const topUsers = await service.getLeaderboard(25);
      console.log('[PinnedPost] loadData: leaderboard count', topUsers?.length ?? 0);
      setLeaderboard(topUsers);
    } catch (e) {
      console.error('[PinnedPost] loadData: error', e);
      setLeaderboard([]);
    } finally {
      console.log('[PinnedPost] loadData: setLoaded(true)');
      setLoaded(true);
      if (splashEnabled) {
  ...
  return (
    <zstack width="100%" height="100%">
      ...
      {mask && <SplashScreen onContinue={hideMask} />}
    </zstack>
  );
}
